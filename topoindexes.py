...
def topoindex_sequential(fac,slope,px):
    ...
    '''
    tp = np.where(slope==-100,-100,np.log((np.where(fac==0,1,fac)*np.power(px,2))/(np.tan(slope))))
    ...
    for i in range(0,len(slope),1):
        for j in range(0,len(slope[0]),1):
            if slope[i,j] == -100:
                ...
                if fac[i,j] == 0:
                    tp[i,j] = np.log((1*px*px)/(np.tan(slope[i,j])+0.0001))
                    # lnhlh[i,j] = 1
                else:
                    tp[i,j] = np.log((fac[i,j]*px*px)/(np.tan(slope[i,j])+0.0001))
        ...
        for j in range(0,len(slope[0]),1):
            if slope[i,j] == -100:
                ...
                if fac[i,j] == 0:
                    ...
                    mtp[i,j] = np.log(np.power(1*px*px,n)/(np.tan(slope[i,j])+0.0001))
                    # lnhlh[i,j] = 1
                else:
                    mtp[i,j] = np.log(np.power(fac[i,j]*px*px,n)/(np.tan(slope[i,j])+0.0001))
                    # tp[i,j] = math.log(((fac[i]*px**2)**n)/(math.tan(slope[i])))
                    # lnhlh[i,j] = np.log(b*(np.power(fac[i,j], n))/hand[i,j])
                    # lnhlh[i,j] = 2
...
def modtopoindex_sequential(fac,slope,px,n):
    ...
    mtp = np.where(slope==-100,-100,np.log(np.power((np.where(fac==0,1,fac)*np.power(px,2)),n)/(np.tan(slope))))
...
def topoIndex(fac, slope, px, ntop, div_col=0, div_row=0):
    ...
    for m in range(0,div_row+1,1):
        for n in range(0,div_col+1,1):
            ...
            TI[mS:mE,nS:nE], MTI[mS:mE,nS:nE] = topoIndex_host(fac[mS:mE,nS:nE],
                                                slope[mS:mE,nS:nE], px, ntop)
    ...
    if i >= 0 and i < len(slope):
        if fac[i] <= -100:
            ...
            if fac[i] == 0:
                ...
                TI[i] =  math.log((fac[i]*px**2)/(math.tan(slope[i]+0.0001))) 
    ...
    if i >= 0 and i < len(slope):
        if fac[i] <= -100:
            ...
            if fac[i] == 0:
                ...
                # MTI[i] = n
                MTI[i] =  math.log(((1*px**2)**n)/(math.tan(slope[i]+0.0001)))
            else:
                MTI[i] =  math.log(((fac[i]*px**2)**n)/(math.tan(slope[i]+0.0001)))
