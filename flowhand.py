"""
Descritores: Flow distance, HAND e GFI
"""
# Anotações:
            ...
            if fdr[i,j] == 255:
                ...
                while(river[y,x] != 1):
                    ...
                    if y == 0 and (fdr[y,x]==32 or fdr[y,x]==64 or fdr[y,x]==128):
                        isnan = 1
                        break
                    elif y ==len(fdist)-1  and (fdr[y,x]==2 or fdr[y,x]==4 or fdr[y,x]==8):
                        isnan = 1
                        break
                    elif x == 0  and (fdr[y,x]==32 or fdr[y,x]== 16 or fdr[y,x]== 8):
                        isnan = 1
                        break
                    elif x ==len(fdist[0])-1  and (fdr[y,x]== 128 or fdr[y,x]== 1 or fdr[y,x]== 2):
                        isnan = 1
                        break
...
@jit
def fdist_indexes_sequential_jit(fdr,river,px, fdist=np.array([[],[]],'float32')):
    '''
    Flow distance to the nearest drainage sequential method. Also preemptively 
    calculates the row and columns required for the matrix division.
        ...
        for n in range(0,div_col+1,1):
            ...
            if div_row > 0:
                if part < (div_col+1)*div_row: #vizinho pra baixo
                    ...
                    if n != div_col:
                        ...
                        bound_b = np.insert(bound_b,nE-(nS+1),fdist[mE,nE])
                    if n != 0:
                ...
                if part > div_col: #vizinho em cima
                    ...
                    if n != div_col:
                        bound_c = np.insert(bound_c,nE-(nS+1),fdist[mS,nE])
                    if n != 0:
            ...
            if div_col > 0:
                if part%(div_col+1) != 0:
                    ...
                    if m != div_row:
                        #Célula diagonal 
                        bound_e = np.insert(bound_e,mE-(mS+1),fdist[mE,nS])
                    if m != 0:
                ...
                if part%(div_col+1) != div_col:
                    ...
                    if m != div_row:
                        bound_d = np.insert(bound_d,mE-(mS+1),fdist[mE,nE])
                    if m != 0:
            ...
            fdist[mS:mE,nS:nE], indices[mS:mE,nS:nE] = fdist_index_host(dem[mS:mE,nS:nE], 
                fdr[mS:mE,nS:nE], river[mS:mE,nS:nE],px,bound,bound_index,out) 
            # print(time.time() - now)
            # now = time.time()

            indices[mS:mE,nS:nE] = index_calculator(indices[mS:mE,nS:nE],mS,nS,col)
...
def hand_calculator(dem,indices):
    ...
    hand = np.where((dem!=-100) & (indices!=-100),dem-dem[indices],-100)
    ...
    for i in range(0,len(dem),1):
        for j in range(0,len(dem[0]),1):
            if dem[i,j] == -100 or indices[i,j] == -100:
                ...
                jj = jj.astype('int')
                hand[i,j] = 0 if dem[i,j] - dem[ii,jj] < 0 else dem[i,j] - dem[ii,jj]
...
def index_calculator(ind,row_start,col_start,col_size):
    ...
    ind = np.where(ind == -100,-100,(np.floor(ind/col)+row_start) * col_size + ind%col + col_start)
    # ind = np.where(ind == -100,-100,(math.floor(ind/col) + row_start) * col_size + ind%col + col_start)

    return ind

def fdist_index_host(dem,fdr,river,px,bound_dist,bound_index,out,blocks=0,threads=0):
    '''
    ...
    fdist_index_device[blocks,threads](fdr, river, px, fdist, indices, col, row, bound_dist,bound_index, out)
...
@cuda.jit
def fdist_index_device(fdr,river,px,fdist,indices,col,row,bound_dist,bound_ind,out):
    '''
        ...
        if fdr[i] <= -100:
            ...
            if river[i] == 1:
                ...
                while(river[pos] != 1):
                    ...
                    elif pos >= (row-1) * col and (fdr[pos] == 2 or fdr[pos] == 4 or fdr[pos] == 8):
                        if out[3] == 0:
                            isnan = 1
                            break
                        else:
