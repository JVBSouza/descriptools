            ...
            for y in range(-1,2,1):
                    for x in range(-1,2,1):
                        ...
                        if x == 0 or y == 0:
                            ...
                            if aux < (dem[i,j] - dem[i+y,j+x])/(px*1.4142):
                                aux = (dem[i,j] - dem[i+y,j+x])/(px*1.4142)
                            else:
                                continue
            slope[i,j] = aux * 100
    return slope
...
def sloper(dem, px):
    ...
    for m in range(0,div_row+1,1):
        for n in range(0,div_col+1,+1):         
            ...
            slope[mS:mE,nS:nE] = slope_host(dem[mS-1+extra[0]:mE+1-extra[3],nS-1+extra[1]:nE+1-extra[2]], px, extra)
